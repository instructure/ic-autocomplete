...
export default Ember.Component.extend({
  ...
  /**
   * Closes the list when focus moves away from the component or any of its
   * children. If there is a valid autocomplete happening, it selects it,
   * otherwise it would be weird to have a valid label inside the text field
   * but not have the autocomplete `value` match.
   *
   * @method closeOnFocusOutAndMaybeSelect
   * @private
   */

  closeOnFocusOutAndMaybeSelect: function() {
    ...
    Ember.run.later(this, function() {
      if (!this.get('element').contains(document.activeElement)) {
        if (autocompletedOption) {
          ...
          // TODO: we should not be sending the same action in two places!
  ...
  bindValue: function() {
    ...
    // TODO: when options are registered, create an id map and lookup values
    // instead of iterating the options
