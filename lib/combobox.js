...
export default Ember.Component.extend({
  ...
  handleKeydown: function(event) {
    ...
    if (event.keyCode == 40) this.focusNext();
    if (event.keyCode == 38) this.focusPrevious();
  }.on('keyDown'),
  ...
  closeOnFocusOut: function(event) {
    ...
    var wasChild = el !== event.target && el.contains(event.target);
    if (wasChild) {
      return;
    }
    this.close();
...
function delegate(childName) {
  var method, child;
  return function fn() {
    if (!child) child = this.get(childName);
    if (!method) {
      for (var key in this) {
        if (this[key] === fn) {
          method = key;
    ...
    return child[method].apply(child, arguments);
  };
}
